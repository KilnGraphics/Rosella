...
import graphics.kiln.rosella.debug.LegacyDebugCallback;
...
/**
 * {@link Rosella} representation of a {@link VkInstance}. Contains a couple of useful things here and there and does most of the work for you.
 */
@Deprecated
public class LegacyVulkanInstance {
    ...
    public LegacyVulkanInstance(List<String> requestedValidationLayers, List<String> requestedExtensions, String applicationName, DebugLogger debugLogger) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (validationLayers) {
                ...
                createInfo
                        .ppEnabledLayerNames(Memory.asPtrBuffer(requestedValidationLayers))
                        .pNext(debugMessengerCreateInfo.address());
            }

            PointerBuffer pVkInstance = stack.mallocPointer(1);
            VkUtils.ok(vkCreateInstance(createInfo, null, pVkInstance));
            ...
            if (validationLayers) {
                ...
                VkUtils.ok(vkCreateDebugUtilsMessengerEXT(rawInstance, debugMessengerCreateInfo, null, messenger));
     ...
     * @param requestedExtensions extensions requested by {@link Rosella} based on user choice and necessity
